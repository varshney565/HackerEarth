...
void solve() {
    ...
    while(l <= r) {
        ...
        for(int i = 1 ; i < n ; i++) {
            ...
            if(abs(a[i]-a[i-1]) <= mid) new_dp[0] = min(dp[0],new_dp[0]);
            if(abs(a[i]-b[i-1]) <= mid) new_dp[0] = min(new_dp[0],dp[1]);

            // for dp[1]
            if(abs(b[i]-b[i-1]) <= mid) new_dp[1] = min(new_dp[1],dp[1] + 1);
            if(abs(b[i]-a[i-1]) <= mid) new_dp[1] = min(new_dp[1],dp[0] + 1);
            dp = new_dp;
        }
        if(min(dp[0],dp[1]) <= k) {
            ans = mid;
