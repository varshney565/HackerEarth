...
void solve() {
    ...
    vector<map<int,bool>> dp(n+1,map<int,bool>());
    int cnt = 0;
	function<bool(int,int)> go = [&](int ind,int L) {
        /////////////////////////////////////////////////////////
        /// For Debugging 
        ...
        for(int i = -1 ; i <= 1 ; i++) {
            int newL   = L+i;
            int newpos = v[ind-1]+newL;
            if(f.find(newpos) != f.end()) {
                int newInd = f[newpos];
                if(dp[newInd].find(newL) != dp[newInd].end()) {
                    continue;
                }
                if(go(newInd,newL)) {
                    res = true;
